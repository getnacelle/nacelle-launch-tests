...
if (!NACELLE_PASSPORT_SECRET) {
  logger.error('No client secret. Set NACELLE_PASSPORT_SECRET environment variable.');
  process.exit(1);
}

if (!FACEBOOK_APP_ID) {
  logger.error('No client facebook id. Set FACEBOOK_APP_ID environment variable.');
  process.exit(1);
}

if (!FACEBOOK_APP_SECRET) {
  logger.error('No client facebook secret. Set FACEBOOK_APP_SECRET environment variable.');
  process.exit(1);
}

if (!GOOGLE_CLIENT_ID) {
  logger.error('No client google id. Set GOOGLE_CLIENT_ID environment variable.');
  process.exit(1);
}

if (!GOOGLE_CLIENT_SECRET) {
  logger.error('No client google secret. Set GOOGLE_CLIENT_SECRET environment variable.');
  process.exit(1);
}