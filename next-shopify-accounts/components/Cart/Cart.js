...
const Cart = () => {
  ...
  const [checkoutSubmited, setCheckoutSubmited] = useState(false);

  useEffect(() => {
    if (checkoutData && !checkoutSubmited) {
      setCheckoutSubmited(true);
      const { processCheckout } = checkoutData.data;
      console.log(checkoutData.data);
      if (accountData && accountData.customer) {
        accountDispatcher({
          type: 'MULTIPASS_LOGIN',
          payload: { returnTo: processCheckout.url }
        });
      } else {
        window.location = processCheckout.url;
      }
    }
  }, [checkoutData, accountData, checkoutSubmited, accountDispatcher]);
