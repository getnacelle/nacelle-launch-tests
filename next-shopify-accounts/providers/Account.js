...
async function accountReducer(state, action) {
  switch (action.type) {
    ...
    case 'MULTIPASS_LOGIN': {
      const newState = await multipassLogin(state, action.payload);
      state = { ...newState };
      console.log('57', newState);
      return newState;
    }
    case 'LOGOUT': {
    ...
    case 'ACTIVATE': {
      const newState = await activate(state, action.payload);
      state = { ...newState };
      console.log('98', newState);
      return newState;
    }
    case 'INIT_PASSPORT': {
      const newState = await initPassport(state);
      state = { ...newState };
      console.log('92', newState);
      return newState;
    }
    case 'FETCH_ORDERS': {
      const newState = await fetchOrders(state);
      state = { ...newState };
      console.log('95', newState);
      return newState;
    }
...
const multipassLogin = async (state, payload) => {
  let newState = { ...state };
  if (process.browser) {
    ...
    try {
      const multipassData = await apiPost('/multipassify', {
        data: { customerData }
      });
...
const activate = async (state, payload) => {
  let newState = { ...state };
  try {
    const { password, customerId, resetToken } = payload;
    const id = Buffer.from(`gid://shopify/Customer/${customerId}`).toString(
      'base64'
    );
    const activationUrl = `https://${myshopifyDomain}/${id}/${resetToken}`;
    ...
    if (customerAccessToken) {
      ...
      return await multipassLogin(newState);
    }
  } catch (error) {
    console.error(error);
    throw error;
...
const authUrl = (strategy) => {
  if (process.browser) {
    return `${serverlessEndpoint}/auth/${strategy}?returnTo=${encodeURIComponent(
...
const initPassport = async (state) => {
  let newState = { ...state };
  newState = { ...newState, facebook: authUrl('facebook') };
  newState = { ...newState, google: authUrl('google') };
  console.log('A', newState);
  const ncl = await getCookie('ncl');
  if (ncl) {
    try {
      const { email, jwt, strategy, accessToken, expiresAt } = ncl;
      // await fetch(`${this.endpoint}/auth/status`, {
      //   withCredentials: true
      // });

      await fetch(`${serverlessEndpoint}/auth/status`, {
        method: 'GET',
        body: { withCredentials: true }
      });

      //TODO move to the component side
      console.log('STRATEGY', strategy);
      // this[strategy].text = this[strategy].text.replace('Login', 'Logged');
      // this[strategy].url = '#';
      if (accessToken) {
        newState = renewCustomerAccessToken(newState, accessToken);
        //TODO move to the component side
        //this.$router.push('/account');
      }
    } catch (error) {
      console.error('error', error);
      removeCookie('ncl_email');
      removeCookie('ncl_strategy');
    }
  }
...
const fetchOrders = async (state) => {
  ...
  try {
    ...
    if (!newState.customer || !newState.customer.id) {
      newState = await fetchCustomer(newState);
    }

    const ordersResponse = await apiPost('/customer-orders', {
      data: { customerID: newState.customer.id }
    });
    newState = { ...newState, orders: ordersResponse };
